...
targets = [
    'Daniel Curl', 'Michael Kuss', 'Melina Voglhuber', 'Allan Thivoz',
    'Joshua Schubert', 'Florian Hamm', 'Camille Picano', 'Clement Turpain',
    'Vincent Roubadia', 'Christina Landowski'
]
...
for target in targets:
    ...
    highest = max([l for l in stats['points_by_level'].keys()], 
                 key=lambda x: LEVEL_ORDER.index(x) if x in LEVEL_ORDER else 99) if stats['points_by_level'] else 'N/A'
    
    european_pct = (stats['points'] / stats['global_points'] * 100) if stats['global_points'] > 0 else 100
    
    # Прогрессия
    levels_with_points = sorted([l for l in stats['points_by_level'].keys()], 
