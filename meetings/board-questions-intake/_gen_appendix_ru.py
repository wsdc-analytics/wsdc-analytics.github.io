...
RU: dict[str, tuple[str, str]] = {
    "JT-1": (
        ...
        "Tigges №1. Методология «done» не выбрана — May/Must в Time in Division это не закрывают. См. TIME_IN_DIVISION_C1_AUDIT.md.",
    ),
    "JT-2": (
        "Длительность для каждого конкурента от последнего поинта в нижнем дивизионе до первого поинта в более высоком",
        "Tigges №2. Пауза last_to_first — отдельный лист (только месяцы; ось ивентов пустая по построению). Не текущий dwell scatter. См. TIME_IN_DIVISION_C1_AUDIT.md.",
    ),
    "JT-3": (
        ...
        "Tigges №3. На сайте: месяцы и ивенты до May / Must (дашборд Time in Division). Буфер 25/50/75% ещё открыт. См. TIME_IN_DIVISION_C1_AUDIT.md.",
    ),
    "JT-4": (
    ...
    "JN-1c": (
        ...
        "Время в Champions / время до дивизиона / финалы (где есть place) = C1. «На скольких ивентах выступали» ≠ известно — только ивенты с поинтом. Реальный compete count → JN-1c-E.",
    ),
    "JN-1c-E": (
        "Статистика по текущим чемпионам с 10+ поинтами: сколько они уже чемпионы, сколько шли до дивизиона, среднее число ивентов и выходов в финал в год",
        "Что сделать: Competitors List / attendance, сджойненный с ID чемпионов, чтобы «events they compete at» = старты, а не ивенты-с-поинтом.",
    ),
    "JN-1d": (
    ...
    "JN-2b": (
        ...
        "Только среди тех, кто уже есть в Points Registry (когда-то получал поинт): нет поинтов после ковида / geo последнего поинта. Кто никогда не scored — невидим. Полный «не соревновались» / Entry no-shows → JN-2b-E.",
    ),
    "PL-1a": (
        "Сколько ивентов проводили Champion и/или Invitational (окна 3 / 6 / 9 лет)",
        "Только Champions и только где в результатах начислялись поинты. «Held» для Invitational из Points Registry так не отвечаем — нужен Score Report / программы / другой источник.",
    ),
    "PL-1c": (
    ...
    "KY-1": (
        ...
        "Сейчас: scored-прогрессия / концентрация среди тех, у кого уже есть поинты (переиспользовать статью). «Большинство танцоров» vs меньшинство point-winners требует всех, кто участвовал → KY-1-E.",
    ),
    "KY-1-E": (
        "Ближе посмотреть на текущую структуру поинтов и тиров и влияние возможных изменений, особенно на прогрессию большинства танцоров (многие поинты забирает одно и то же небольшое меньшинство)",
        "Что сделать: join Competitors List, чтобы знаменатель = все, кто зашёл (majority on the floor), а не только scored ID; тогда доля вошедших с поинтами / прогрессия большинства без поинтов.",
    ),
    "JC-5b": (
        "Есть ли у комитета идеи другой аналитики, полезной для заявок на новые ивенты (New Event Applications)?",
        "Стратегический / initiative-вопрос про NEA — не C1-deliverable из Points Registry. Product NEA за Board/ops; комитет советует только по мандату (фильтр Kay).",
    ),
    "YV-1": (
    ...
    "JT-5": (
        ...
        "Tigges №5. Вопрос про tier mix, не точный Entry headcount — отвечаем полосами размера из реестра (наличие дивизиона + tier по scored-размеру номинации).",
    ),
    "JT-7": (
    ...
    "JN-2b-E": (
        "Сколько людей не соревновались (или не получили поинт, если нет регистрационных данных) после ковида — в каких штатах / странах сильнее спад",
        "Что сделать: Entry-списки по ивентам + identity — кто заходил, но никогда не scored, и настоящие no-shows; из одного Points Registry это не вывести.",
    ),
    "JN-3a-E": (
        "Число добавленных ивентов YOY по странам с количеством конкурентов на каждом ивенте",
    ...
    "JC-5d": (
        ...
        "Какие метрики для исключений NEA. Полный запрос Jen (JN-3c) = C4 local WCS; WSDC-прокси потребовал бы home city/state + радиус (отдельный дизайн).",
    ),
    "JN-3c": (
        "Популяция WCS в радиусе 100 миль вокруг каждого ивента",
        "Community / локальный WCS, не реестр WSDC: центр = venue ивента; счёт = локальная популяция вне контура WSDC. То же семейство, что JC-5c / JC-1–2. (WSDC home-city × радиус — более узкий прокси, только если Jen согласится.)",
    ),
    "PL-3": (
