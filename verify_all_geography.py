...
# Находим все блоки с именами танцоров
pattern = r'<strong>([^<]+)</strong>.*?<strong>География:</strong>\s*([^<]+)'
...
for dancer_name_raw, geo_text in matches:
    ...
    # Пропускаем если это не имя танцора
    if not dancer_name or len(dancer_name) < 3:
        continue
    ...
    if not dancer_id:
        ...
        for name, did in name_to_dancer_id.items():
            if dancer_name.lower() in name.lower() or name.lower() in dancer_name.lower():
                dancer_id = did
                dancer_name = name
                break
    
    if not dancer_id:
        print(f"\n⚠️  Не найден dancer_id для: {dancer_name_raw}")
        continue
    ...
    if "все поинты" in stated_geo.lower() and abs(points_by_region['european'] / total_points - 1.0) > 0.01:
