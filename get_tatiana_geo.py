...
top_event, top_data = max(events_data.items(), key=lambda x: x[1]['points'])
location = f"{top_data['city']}, {top_data['country']}" if top_data['city'] else top_data['country']
print(f"Топ-ивент: {top_event} ({location}) - {int(top_data['points'])} поинтов")
