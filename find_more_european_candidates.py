...
# Уже упомянутые (включая тех, что в европейском блоке)
mentioned = {
...
# Кандидаты из европейского топа (по всем метрикам)
candidates = [
...
filtered.sort(key=lambda x: (x[1]['points'], x[1]['wins'], len(x[1]['events'])), reverse=True)

print("Топ кандидатов для заметок (не упоминались ранее):")
print("="*80)
for i, (name, st) in enumerate(filtered[:5], 1):
    ...
    print(f"{i}. {name}:")
    print(f"   Поинты: {st['points']:.0f}, Победы: {st['wins']}, Ивенты: {len(st['events'])}")
